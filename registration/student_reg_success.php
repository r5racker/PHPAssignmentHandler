    ...
    if(/*empty($_POST['username'])||strlen($_POST['username'])>11*/0){
        ...
        if(empty ($_FILES["student_image"]["name"]))
        {
            ...
            if($row["student_email"]==$student_email){
                header('Location:../login.php?message=you are already registered&user_email='.$_POST["student_email"]);
            }
            ...
            if(! (move_uploaded_file($_FILES["student_image"]["tmp_name"], $target_location))){//|($_FILES["student_image"]["size"]>300000)|(strcmp($_FILES["student_image"]["type"],'image/png')!=0)){
                ...
                echo "please move back and check file properly";
                die();
            }
            else
            {
                ...
                try{
                    ...
                    . "(student_email,student_first_name,student_middle_name,student_last_name,student_dob,student_gender,student_semester,student_address,student_address2,student_city,student_state,student_zip,student_mobile_no,student_id_no) "
                    . "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                    ...
                    $prepared_sql->execute(array($student_email,$_POST["student_first_name"],$_POST["student_middle_name"],$_POST["student_last_name"],$date,$_POST["student_gender"],$_POST["student_semester"],$_POST["student_address"],$_POST["student_address2"],$_POST["student_city"],$_POST["student_state"],$_POST["student_zip"],$_POST["student_mobile_no"],$_POST["student_id_no"]));
                    $prepared_sql2->execute(array($_POST["student_email"],$_POST["student_password"],"stu"));
                    echo "Data is inserted successfully";

                    session_start();
                    $_SESSION['user_email']=$_POST["student_email"];
                    echo "successfully registered";
                    header('Location:../Student/StudentHome.php?user_email='.$_POST["student_email"]);
                }
